...
Particle::Particle(Problem *problem, Configuration *config, int identifier, long int iteration)
{
	...
	lmedian.x = new double[size];
	current.eval = LDBL_MAX;
	pbest.eval = LDBL_MAX;
	lbest.eval = LDBL_MAX;
	lmedian.eval = LDBL_MAX;
...
void Particle::assertValueInbounds(Configuration* config, double &value){
	//Avoid infities and not a number errors
	if (isinf(value) || isnan(value))
		randomPointInBounds(config, value);
	//Avoid too large values for these two unbound problems
	if(config->getCompetitionID() == CEC05 && config->getProblemID() == SHIFTED_ROTATED_GRIEWANK_CEC05){
		if (value > 600.0)
			value = 600.0;
		if (value < 0.0)
	...
	//Correct out of bounds solutions
	else{
		if (value < config->getMinInitBound())
			value = config->getMinInitBound();
		if (value > config->getMaxInitBound())
			value = config->getMaxInitBound();
	}
}

void Particle::randomPointInBounds(Configuration* config, double &point){
	if(config->getCompetitionID() == CEC05 && config->getProblemID() == SHIFTED_ROTATED_GRIEWANK_CEC05)
		point = RNG::randVal(0.0,600.0);
	else if(config->getCompetitionID() == CEC05 && config->getProblemID() == ROTATED_HYBRIDCOMPOSITION4_NO_BOUNDS)
		point = RNG::randVal(2.0,5.0);
	else
		point = RNG::randVal(config->getMinInitBound(),config->getMaxInitBound()); //random values within the bounds of the function
}
...
void Particle::initializePosition(Configuration *config, long int iteration, bool updatePbest)
{
	...
	if (config->getParticleInitType() == PARTICLE_INIT_MODEL)
	{ // Only available for dynamic PopCS and after the first iteration
		initUniform(config);
		...
		int lastLevelComplete, int solImproved)
{
	...
	for (int i = 0; i < size; i++)
	{
		...
		assertValueInbounds(config, current.x[i]);
		randomPointInBounds(config, current.x[i]);
	}
	if (id == -1){
		cout << "\tvec::velocity[i].p[].pb: [";
		for (int i = 0; i < size; i++)
		{
			cout << fixed << velocity[i] << " ";
		}
		cout << "]" << endl;
		cout << "\tvec::current.x[i].p[].pb: [";
		for (int i = 0; i < size; i++)
		{
...
void Particle::computeSubtractionPerturbationRotation(
		Configuration *config,
		double vect_distribution[], // This is the vector we aim to compute here
		long int iteration,
		int solImprov)
{
	...
	if (config->getDistributionNPP() == DIST_COYOTE)
	{
		...
		{
			//delta1[i] = neighbors[alpha]->pbest.x[i] - neighbors[rc1]->pbest.x[i];
			//delta2[i] = neighbors[beta]->pbest.x[i] - neighbors[rc2]->pbest.x[i];
		...
		/*cout << "\tvec::delta1.p[" "].pb: [";
		for (int i = 0; i < size; i++)
		{
			cout << fixed << delta1[i] << " ";
		}
		cout << "]" << endl;
		cout << "\tvec::delta2.p[" "].pb: [";
		for (int i = 0; i < size; i++)
		{
			cout << fixed << delta2[i] << " ";
		}
		...
		for (unsigned int i = 0; i < size; i++)
		{
			...
			vect_distribution[i] = r1 * delta1[i] + r2 * delta2[i];
			//cout << "\n:::::REACH THIS POINT:::::" << endl;
		...
		/*cout << "\tvec::vect_distribution.p[].pb: [";
		for (int i = 0; i < size; i++)
		{
			cout << fixed << vect_distribution[i] << " ";
		}
		cout << "]" << endl;*/
...
void Particle::getAdditiveStochasticDNPP(Configuration *config, double vect_distribution[], vector<vector<double>> &vect_PbestMinusPosition)
{
	...
	case Q_STANDARD:
		for (int i = 0; i < size; i++)
		{
			vect_distribution[i] = ((phi_1 * vect_PbestMinusPosition[p1Index][i]) +
					(phi_2 * vect_PbestMinusPosition[p2Index][i])) /
					(phi_1 + phi_2);
	...
	case Q_DISCRETE_2:
		for (int i = 0; i < size; i++)
		{
			...
			vect_distribution[i] = (((1 + rndBernoulli) * vect_PbestMinusPosition[p1Index][i]) +
					((1 - rndBernoulli) * vect_PbestMinusPosition[p2Index][i])) /
					2;
			vect_distribution[i] -= current.x[i];
		}
		break;
...
double Particle::applyInformedPerturbation(Configuration *config, double pertMagnitude[], double pos_xi, int index, long int iteration)
{
	...
	if (config->getPerturbation1CS() == PERT1_GAUSSIAN)
	{
		...
		iteration == 1 ? returnVal = pos_xi + RNG::randGauss(1) : returnVal = pos_xi + RNG::randGauss(1) * pertMagnitude[index];
	}
...
void Particle::getRandomAdditivePerturbation(Configuration *config, double vect_perturbation[])
{
	...
	else if (config->getPerturbation2CS() == PERT2_NOISY)
	{ // Additional noisy
		for (int i = 0; i < size; i++)
		{
...
void Particle::computeRndMatrix(Configuration *config, double **rndMatrix, int RmatrixType, double angle)
{
	...
	if (RmatrixType == MATRIX_DIAGONAL)
	{ // Random diagonal matrix
		for (int i = 0; i < size; i++)
			rndMatrix[i][i] = RNG::randVal(0, 1);
	}
	if (RmatrixType == MATRIX_LINEAR)
	{ // Random linear matrix
		double rndVal = RNG::randVal(0, 1);
		for (int i = 0; i < size; i++)
			rndMatrix[i][i] = rndVal;
	}
	if (RmatrixType == MATRIX_RRM_EXP_MAP)
	{ // Random rotation matrix using exponential method
	...
	if (RmatrixType == MATRIX_RRM_EUCLIDEAN_ONE)
	{ // Random rotation matrix using Euclidean rotation (ONLY ONE PLANE)
	...
	if (RmatrixType == MATRIX_RRM_EUCLIDEAN_ALL)
	{ // Random rotation matrix using Euclidean rotation (ALL PLANES)
		// No need to generate a matrix, the multiplication of the vector by the rotation matrix is done directly in multiplyVectorByRndMatrix()
		for (int i = 0; i < size; i++)
			rndMatrix[i][i] = 1.0;
	}
	...
	if (RmatrixType == MATRIX_GROUPED_INCREASING)
	{ // Random diagonal matrix using the increasing stochastic scaling grouping
		double *theMatrixGI;
...
double Particle::getAnAngle(Configuration *config, int solImprov, long int iteration)
{
	...
	if (config->getAngleCS() == ANGLE_ADAPTIVE)
	{ // draw the angle from a Normal distribution with µ=0 using an adaptive s.d.
		...
		if (iteration == 1)
		{ // define simpSwarm if it does not exist
