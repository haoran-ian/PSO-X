...
Swarm::Swarm(Problem *problem, Configuration *config)
{
	...
	{
		(config->getPopulationCS() == POP_CONSTANT) ? createHierarchical(config->getBranchingDegree(), swarm.size()) : createHierarchical(config->getBranchingDegree(), config->getFinalPopSize());
	}
...
void Swarm::computeRandomMatrixGroupedIncreasing(Configuration *config, long int iteration)
{
	...
	if (config->getRandomMatrix() == MATRIX_GROUPED_INCREASING)
	{
		...
		if (groups < 1)
			groups = 1;
		if (groups > config->getProblemDimension())
			groups = config->getProblemDimension();
...
void Swarm::getInformants(Configuration *config, int pPosinSwarm, long int iteration)
{
	...
	else
	{
		...
		if (config->getModelOfInfluence() == MOI_RANKED_FI)
		{
			...
			for (unsigned int i = 0; i < TMP_vect.size(); i++)
			{ // rows
...
void Swarm::updateTopologyConnections(Configuration *config, long previous_size, long int iteration)
{
	...
	if (previous_size > 0)
	{ // PARTICLES WERE ADDED -- EXTEND THE TOPOLOGY
		if (config->getTopology() == TOP_HIERARCHICAL)
		{
		...
		if (config->getTopology() == TOP_TIMEVARYING)
		{
			...
			{ // RING -- After the topology update period is finished
				for (unsigned int i = 0; i < swarm.size(); i++)
				{
					swarm.at(i)->neighbors.clear();
				}
...
void Swarm::reinitializeParticlePosition(Configuration *config, long int iteration)
{
	...
	if (iteration % reinitSchedule == 0)
	{
		...
		if (fabs(previousgBestEval - best_particle->getPbestEvaluation()) < OBJECTIVE_FUNCTION_CHANGE_THRESHOLD)
		{
			...
			for (unsigned int i = 0; i < swarm.size(); i++)
			{
				if (swarm.at(i)->getID() != best_particle->getID())
				{ // exclude gBest
...
/* Topologies */
void Swarm::createFullyConnectedTopology()
{ // All particles are neighbor among them
	for (unsigned int i = 0; i < swarm.size(); i++)
	{
		for (unsigned int j = 0; j < swarm.size(); j++)
		{
...
void Swarm::createRingTopology()
{ // Every particle is neighbor of the adjacent particles
	if (swarm.size() > 2)
	{
		int a;
		int b;
...
void Swarm::createWheelTopology()
{ // Particles are neighbors of one central particle
...
void Swarm::createRandomEdge()
{ // Random edge topology
	long int randomEdge;
...
void Swarm::updateTimeVaryingTopology(Configuration *config, long int iterations)
{
	...
	if ((iterations > 0) && (iterations % config->getTopologyUpdatePeriod() == 0) &&
		((config->getEsteps() < swarm_size - 3 && (config->getPopulationCS() == POP_CONSTANT)) || ((config->getEsteps() > 0 && swarm.size() > 4) && config->getPopulationCS() != POP_CONSTANT)))
	{
		...
		while (removals < target && averageConnections > 3 && emergyExit == false)
		{
			...
			{ // vector::at throws an out-of-range
				for (unsigned int i = 0; i < swarm.size(); i++)
				{
					...
					if (swarm.at(particleIndex)->neighbors.size() > 3)
					{ // 3 because a particle is a neighbor to itself
...
void Swarm::addParticlesInLastLevel(int first, int last, int branching)
{
	...
	{ // particles to add
		...
		{ // maxWidth of the level
			if (hierarchy.at(lastLevelComplete + 1).at(width + (nodesCounter * (branching - 1)) + iterCount) == -2)
...
void Swarm::printTree(int branching)
{
	...
	cout << "\n\nRoot node: " << endl;
	cout << "\t\t" << hierarchy.at(0).at(0) << "->" << swarm.at(hierarchy.at(0).at(0))->getParent() << endl
		 << "Level 1: " << endl;
	// reset indexes to traverse the entire tree
	int h = 1;
	int width = 0;
...
double Swarm::computeOmega1(Configuration *config, long int iteration, long int posIndex, bool newIteration)
{
	...
	if (newIteration)
	{
		...
		else if (config->getOmega1CS() == IW_VELOCITY_BASED)
		{
			...
			{
				(config->getOmega1() - lambda) >= config->getInitialIW() ? config->setOmega1(config->getOmega1() - lambda) : config->setOmega1(config->getInitialIW());
			}
			else
			{
				(config->getOmega1() + lambda) >= config->getFinalIW() ? config->setOmega1(config->getFinalIW()) : config->setOmega1(config->getOmega1() + lambda);
			}
			OmegaToReturn = config->getOmega1();
		}
